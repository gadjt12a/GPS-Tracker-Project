...
//    }
       ...
       reject queries without a path component). */
    {
        ...
        /* Peak horizontal g since the last ping — the field data that tunes
           HARSH_EVENT_G. Suppressed when negligible to keep parked pings clean. */
        char g_part[20] = "";
...
SUCCESS:
    ...
    /* Clear only the alarm source that was actually in this ping's URL; a
       pending alarm from the other source rides the next ping. Failed sends
       keep both for the retry. */
    if (harsh_alarm_in_flight) { harsh_alarm = 0; harsh_alarm_in_flight = 0; }
    else                       { power_alarm = 0; }
    gmax_since_ping = 0; // peak was reported; restart the measurement window
    ClearEventCache();
