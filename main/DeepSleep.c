...
void SleepWakeupReason(void)
{
    struct timeval now;
    // vTaskDelay(5000 / portTICK_PERIOD_MS);
    ESP_LOGI(TAG,"Printing Wakeup Reason......");
    ...
    switch (esp_sleep_get_wakeup_cause()) {
        ...
        case ESP_SLEEP_WAKEUP_TIMER: {
            ESP_LOGW(TAG,"Wake up from timer. Time spent in deep sleep: %dms\n", sleep_time_ms);
            break;
        }
#ifdef CONFIG_EXAMPLE_TOUCH_WAKEUP
        case ESP_SLEEP_WAKEUP_TOUCHPAD: {
            ESP_LOGW(TAG,"Wake up from touch on pad %d\n", esp_sleep_get_touchpad_wakeup_status());
            break;
        }
        ...
        case ESP_SLEEP_WAKEUP_ULP: {
            ESP_LOGW(TAG,"Wake up from ULP\n");
            int16_t diff_high = (int16_t) ulp_data_read(3);
            int16_t diff_low = (int16_t) ulp_data_read(4);
            if (diff_high < 0) {
                ESP_LOGW(TAG,"High temperature alarm was triggered\n");
            } else if (diff_low < 0) {
                ESP_LOGW(TAG,"Low temperature alarm was triggered\n");
            } else {
...
void EnterDeepSleep(void)
{
    ...
    if (esp_sleep_get_wakeup_cause() != ESP_SLEEP_WAKEUP_UNDEFINED) {
        ESP_LOGW(TAG,"ULP did %d temperature measurements in %d ms\n", ulp_data_read(1), sleep_time_ms);
        ESP_LOGW(TAG,"Initial T=%d, latest T=%d\n", ulp_data_read(0), ulp_data_read(2));
    }
#endif // CONFIG_IDF_TARGET_ESP32
#endif // CONFIG_EXAMPLE_ULP_TEMPERATURE_WAKEUP
    ...
    uint64_t wakeup_time_sec = HEART_BEAT_INTERVAL;
    uint64_t wakeup_time_microsec = wakeup_time_sec * 1000000;
    ESP_LOGW(TAG,"Enabling timer wakeup, %lld s,%lld us\n", wakeup_time_sec,wakeup_time_microsec);
    esp_sleep_enable_timer_wakeup(wakeup_time_microsec);

#if CONFIG_EXAMPLE_EXT0_WAKEUP
    const int ext_wakeup_pin_0 = 3;

    ESP_LOGW(TAG,"Enabling EXT0 wakeup on pin GPIO%d\n", ext_wakeup_pin_0);
    ...
    ESP_LOGW(TAG,"Enabling EXT1 wakeup on pins GPIO%d, GPIO%d\n", ext_wakeup_pin_1, ext_wakeup_pin_2);
...
#  endif //CONFIG_EXAMPLE_EXT1_USE_INTERNAL_PULLUPS
    ...
    ESP_LOGW(TAG,"Denoise function init\n");
    /* Filter setting */
    ...
    ESP_LOGW(TAG,"touch pad filter init %d\n", TOUCH_PAD_FILTER_IIR_8);
    /* Set sleep touch pad. */
    ...
    ESP_LOGW(TAG,"Touch pad #%d average: %d, wakeup threshold set to %d\n",
        TOUCH_PAD_NUM9, touch_value, (uint32_t)(touch_value * 0.1));
#endif
    ESP_LOGW(TAG,"Enabling touch pad wakeup\n");
...
#ifdef CONFIG_EXAMPLE_ULP_TEMPERATURE_WAKEUP
#if CONFIG_IDF_TARGET_ESP32
    ESP_LOGW(TAG,"Enabling ULP wakeup\n");
...
static void calibrate_touch_pad(touch_pad_t pad)
{
    ...
    if (avg < min_reading) {
        ESP_LOGW(TAG,"Touch pad #%d average reading is too low: %d (expecting at least %d). "
