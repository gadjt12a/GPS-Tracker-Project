...
New attributes on non-live-fix pings: `gpsage` (seconds since last real fix, −1 = never) and `gpsrec` (recovery attempts since boot).
...
Attributes: `hmin` (min free heap since boot), `hcnt` (events passing the gates), `hraw` (raw sensor interrupts before gating — `hraw > 0` with `hcnt = 0` means the gates are wrong, `hraw = 0` means the sensor isn't triggering).
