...
Attributes: `hmin` (min free heap since boot), `hcnt` (events passing the gates), `hraw` (raw sensor interrupts before gating — `hraw > 0` with `hcnt = 0` means the gates are wrong, `hraw = 0` means the sensor isn't triggering), plus `lisreg`/`i2src` from 2.3.41 (the sensor's own config read back at ping time).

**Status: detection still unconfirmed.** `hraw` has stayed 0 across every test drive so far. 2.3.41's readback settles what it *isn't* — the registers come back exactly as intended (`33,57,B7,60,08,0A,2A,19,1E`), so the LIS3DH is correctly configured and stays that way. No calibration data exists yet, so 400 mg / 300 ms remain untested defaults. The next step is to bisect rather than infer: drop `INT2_THS` to a trivially triggerable value on a bench unit and shake it — if `hraw` still doesn't move, the interrupt path is broken; if it does, this is threshold tuning. Note the trial is also gated on GNSS: `hcnt`'s speed gate needs real GPS speed.
