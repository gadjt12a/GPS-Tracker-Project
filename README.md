...
**Status (2.3.50): braking is detected. Tuning out road noise — now via duration, not amplitude.**
...
**Why duration and not a higher threshold.** The five plausible brakes measure 0.15–0.24 g GPS-averaged, i.e. at or *below* the 0.25 g threshold the noise is already clearing — so the two populations overlap in amplitude and raising `HARSH_EVENT_G` removes the signal before the noise. They separate cleanly on duration instead: braking is sustained 1–3 s, a road jolt is under 100 ms. The gate had been doing nothing anyway, since 100 ms truncated to 2 ticks (80 ms) at 25 Hz.
