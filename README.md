...
```
INT2_THS      = 25 counts × 16 mg = 400 mg  (HARSH_EVENT_G)
INT2_DURATION = 30 counts × 10 ms = 300 ms  (HARSH_EVENT_MS)
```

Both derived from the `HARSH_*` macros in `SCI.h` so they can't drift apart. Both generators share the single wired INT1 pin (`CTRL_REG3 = 0x60`) and both are latched (`CTRL_REG5 = 0x0A`), so the existing ~1 Hz INT1 poll can't miss a 300 ms event. Events are classified as `hardBraking` / `hardAcceleration` / `hardCornering` from the track-derived speed trend, and delivered as `alarm=` on a forced ping.
...
**Status: the design premise is measurably false.** `hraw` has stayed 0 across every test drive. 2.3.41's readback settled what it *isn't* — the registers come back exactly as intended (`33,57,B7,60,08,0A,2A,19,1E`), so the sensor is correctly configured and stays that way.
...
- **The INT1 poll is not ~1 Hz — it is roughly once every 11 minutes**, even while driving. Every comment describing "the ~1Hz poll in StartMainTask" (including the one justifying latching at `main.c:1006`) is wrong, and the claim that latching makes the poll "safe" rests on that false rate.
...
This is the first explanation derived from measurement rather than inference, after four inferred ones were wrong (stack/heap/I2C 2.3.35, HPF cutoff 2.3.38, re-init state reset 2.3.39, register drift 2.3.41). It is still a hypothesis until a fix is verified in the field — but the two rates are measured, not argued.

The fix is small: the throttled re-init should test bit 6 and call `HarshEventDetected()` before discarding `INT2_SRC`, turning it into a third poll site instead of a destroyer. Why `qpoll` never fires is a separate defect worth understanding rather than patching around. Note the trial is also gated on GNSS — `hcnt`'s speed gate needs real GPS speed.
