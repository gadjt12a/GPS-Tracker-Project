...
INT2_DURATION =  2 counts × 40 ms =  80 ms  (HARSH_EVENT_MS,    v2.3.46/48)
CTRL_REG1     = 0x37 = 25 Hz ODR            (LIS3DH_ODR_HZ,     v2.3.48)
```

All three derived from macros in `SCI.h` so they can't drift apart. **ODR is not just a sample rate** — it sets the `DURATION` tick (1/ODR) *and* the high-pass cutoff (ODR/400 at `HPCF=11`), which is why 2.3.48 changed it. At 25 Hz, 100 ms is 2.5 ticks and truncates to 80 ms. Both generators share the single wired INT1 pin (`CTRL_REG3 = 0x60`) and both are latched (`CTRL_REG5 = 0x0A`), so an INT1 poll can't miss a short event. Events are classified as `hardBraking` / `hardAcceleration` / `hardCornering` from the track-derived speed trend, and delivered as `alarm=` on a forced ping.
...
**Status (2.3.48): the chain works and fires on real events — but not yet on braking.** The prime suspect is now the high-pass filter, and 2.3.48 is the test.

On the 2026-08-06 drive (2.3.47, 0.25 g) the detector produced its first ever field alarm: `hraw` 0 → 2, `hcnt=1`, `alarm=hardBraking` delivered and logged by Traccar. But it did **not** fire on the deliberate hard stops. Two measured decelerations — **0.409 g** and **0.364 g** — left `hraw` at 0, with `ipoll` climbing eight polls past each event and the interrupt latched, so the sensor genuinely never triggered. A 0.409 g brake cannot fail a 0.25 g threshold, so the threshold was never the blocker. What *did* fire, minutes later, had `i2src` 0x65/0x56 — ZH and ZL, the vertical axis — with no deceleration near it, i.e. a bump. `hardBraking` is the label because classification comes from the GPS speed trend, not the axis. See the 2.3.48 row in Version History for the mechanism and the falsifiable prediction.
